...
for comp in list_cmp:
	...
	os.system("(get-content " + syn_comp + ") -replace 'TOP_DESIGN','" + comp + "' | set-content " + syn_comp)

	if conn_type != "m": #non-mixed case
		input, output = pins(conn_type, conn_type)
		#os.system("sed -i 's/INPUT_PIN/"+ input  +"/g' "+syn_comp)
		os.system("(get-content " + syn_comp + ") -replace 'INPUT_PIN','" + input + "' | set-content " + syn_comp)
		#os.system("sed -i 's/OUTPUT_PIN/"+ output  +"/g' "+syn_comp)
		os.system("(get-content " + syn_comp + ") -replace 'OUTPUT_PIN','" + output + "' | set-content " + syn_comp)
		#os.system("sed -i 's/_CONN//g' "+syn_comp)
		os.system("(get-content " + syn_comp + ") -replace '_CONN','' | set-content " + syn_comp)
		...
		for ind in range(len(input)):
			...
			os.system("(get-content " + syn_comp_ind + ") -replace 'INPUT_PIN','" + in_ind + "' | set-content " + syn_comp_ind)
			#os.system("sed -i 's/OUTPUT_PIN/"+ out_ind  +"/g' "+syn_comp_ind)
			os.system("(get-content " + syn_comp_ind + ") -replace 'OUTPUT_PIN','" + out_ind + "' | set-content " + syn_comp_ind)
			#os.system("sed -i 's/_CONN/_" +  in_ind[0] + out_ind[0]  +   "/g' "+syn_comp_ind)
			os.system("(get-content " + syn_comp_ind + ") -replace '_CONN','_" + in_ind[0] + out_ind[0] + "' | set-content " + syn_comp + syn_comp_ind)
