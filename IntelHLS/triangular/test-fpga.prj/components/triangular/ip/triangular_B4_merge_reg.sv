...
module triangular_B4_merge_reg (
    ...
    // triangular_B4_merge_reg_valid_reg_and_stall_in(LOGICAL,7)
    ...
    // triangular_B4_merge_reg_data_reg_2_x(REG,12)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // triangular_B4_merge_reg_data_reg_1_x(REG,11)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // triangular_B4_merge_reg_data_reg_0_x(REG,10)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
