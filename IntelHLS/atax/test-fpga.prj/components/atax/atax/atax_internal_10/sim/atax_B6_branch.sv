...
module atax_B6_branch (
    ...
    wire [7:0] atax_B6_branch_data_pack_q;
    wire [0:0] atax_B6_branch_valid_and_q;
    wire [6:0] c_i7_01_q;
    ...
    assign element_extension0_q = {c_i7_01_q, in_c0_exe1126};

    // atax_B6_branch_data_pack(BITJOIN,3)
    assign atax_B6_branch_data_pack_q = element_extension0_q;

    // adapt_scalar_trunc2(ROUND,2)
    assign adapt_scalar_trunc2_in = atax_B6_branch_data_pack_q[0:0];
    assign adapt_scalar_trunc2_q = adapt_scalar_trunc2_in[0:0];

    // out_c0_exe1126(GPOUT,14)
    assign out_c0_exe1126 = adapt_scalar_trunc2_q;

    // out_stall_out(GPOUT,15)
    assign out_stall_out = in_stall_in_0;

    // atax_B6_branch_valid_and(LOGICAL,5)
    assign atax_B6_branch_valid_and_q = in_valid_in & in_c0_exe716616;

    // out_valid_out_0(GPOUT,16)
    assign out_valid_out_0 = atax_B6_branch_valid_and_q;
