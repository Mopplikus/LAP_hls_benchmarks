...
// SystemVerilog created from matvec_B7_branch
...
module matvec_B7_branch (
    ...
    wire [0:0] matvec_B7_branch_enable_q;
    wire [0:0] matvec_B7_branch_enable_not_q;
    ...
    begin
        ...
        else if (matvec_B7_branch_enable_q == 1'b1)
        begin
    ...
    // matvec_B7_branch_enable(LOGICAL,13)
    assign matvec_B7_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // c0_exe2120_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_c0_exe2120(GPOUT,21)
    assign out_c0_exe2120 = c0_exe2120_reg_q;

    // c0_exe3121_reg(REG,3)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_c0_exe3121(GPOUT,22)
    assign out_c0_exe3121 = c0_exe3121_reg_q;

    // c0_exe5123_reg(REG,5)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // out_c0_exe5123(GPOUT,23)
    assign out_c0_exe5123 = c0_exe5123_reg_q;

    // matvec_B7_branch_enable_not(LOGICAL,14)
    assign matvec_B7_branch_enable_not_q = ~ (matvec_B7_branch_enable_q);

    // out_stall_out(GPOUT,24)
    assign out_stall_out = matvec_B7_branch_enable_not_q;
