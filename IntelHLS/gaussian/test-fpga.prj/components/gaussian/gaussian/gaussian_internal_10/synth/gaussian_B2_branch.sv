...
module gaussian_B2_branch (
    ...
    wire [0:0] VCC_q;
    reg [31:0] add40_push172_reg_q;
    wire [0:0] cmp21826_cmp_q;
    ...
    // cmp21826_cmp(LOGICAL,3)
    assign cmp21826_cmp_q = ~ (in_cmp21826);

    // valid_out_1_and(LOGICAL,24)
    assign valid_out_1_and_q = in_valid_in & cmp21826_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_cmp21826;
    ...
    // gaussian_B2_branch_enable(LOGICAL,4)
    assign gaussian_B2_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // add40_push172_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            add40_push172_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (gaussian_B2_branch_enable_q == 1'b1)
        begin
            add40_push172_reg_q <= in_add40_push172;
        end
    end

    // out_add40_push172(GPOUT,17)
    assign out_add40_push172 = add40_push172_reg_q;

    // gaussian_B2_branch_enable_not(LOGICAL,5)
