...
module kernel_2mm_B10_branch (
    input wire [0:0] in_c0_exe420210,
    input wire [0:0] in_c0_exe620412,
    ...
    wire [0:0] VCC_q;
    reg [0:0] c0_exe420210_reg_q;
    wire [0:0] c0_exe620412_cmp_q;
    ...
    // c0_exe620412_cmp(LOGICAL,3)
    assign c0_exe620412_cmp_q = ~ (in_c0_exe620412);

    // valid_out_1_and(LOGICAL,24)
    assign valid_out_1_and_q = in_valid_in & c0_exe620412_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe620412;
    ...
    // kernel_2mm_B10_branch_enable(LOGICAL,9)
    assign kernel_2mm_B10_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // c0_exe420210_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe420210_reg_q <= $unsigned(1'b0);
        end
        else if (kernel_2mm_B10_branch_enable_q == 1'b1)
        begin
            c0_exe420210_reg_q <= in_c0_exe420210;
        end
    end

    // out_c0_exe420210(GPOUT,17)
    assign out_c0_exe420210 = c0_exe420210_reg_q;

    // kernel_2mm_B10_branch_enable_not(LOGICAL,10)
