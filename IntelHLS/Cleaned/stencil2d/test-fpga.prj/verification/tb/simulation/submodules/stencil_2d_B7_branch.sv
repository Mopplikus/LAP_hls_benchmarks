...
module stencil_2d_B7_branch (
    ...
    wire [0:0] VCC_q;
    reg [0:0] c0_exe102825_reg_q;
    reg [0:0] c0_exe112838_reg_q;
    wire [0:0] c0_exe1328512_cmp_q;
    ...
    // c0_exe1328512_cmp(LOGICAL,4)
    assign c0_exe1328512_cmp_q = ~ (in_c0_exe1328512);

    // valid_out_1_and(LOGICAL,30)
    assign valid_out_1_and_q = in_valid_in & c0_exe1328512_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe1328512;
    ...
    // c0_exe102825_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe102825_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B7_branch_enable_q == 1'b1)
        begin
            c0_exe102825_reg_q <= in_c0_exe102825;
        end
    end

    // out_c0_exe102825(GPOUT,19)
    assign out_c0_exe102825 = c0_exe102825_reg_q;

    // c0_exe112838_reg(REG,3)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe112838_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B7_branch_enable_q == 1'b1)
        begin
            c0_exe112838_reg_q <= in_c0_exe112838;
        end
    end

    // out_c0_exe112838(GPOUT,20)
    assign out_c0_exe112838 = c0_exe112838_reg_q;
