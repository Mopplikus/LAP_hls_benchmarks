...
module stencil_2d_B8_branch (
    ...
    wire [0:0] VCC_q;
    reg [0:0] c0_exe102826_reg_q;
    reg [0:0] c0_exe112839_reg_q;
    reg [0:0] c0_exe1228411_reg_q;
    reg [0:0] c0_exe1328513_reg_q;
    reg [31:0] c0_exe1428615_reg_q;
    wire [0:0] c0_exe1528716_cmp_q;
    reg [31:0] c0_exe92813_reg_q;
    ...
    // c0_exe1528716_cmp(LOGICAL,7)
    assign c0_exe1528716_cmp_q = ~ (in_c0_exe1528716);

    // valid_out_1_and(LOGICAL,42)
    assign valid_out_1_and_q = in_valid_in & c0_exe1528716_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe1528716;
    ...
    // c0_exe102826_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe102826_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B8_branch_enable_q == 1'b1)
        begin
            c0_exe102826_reg_q <= in_c0_exe102826;
        end
    end

    // out_c0_exe102826(GPOUT,27)
    assign out_c0_exe102826 = c0_exe102826_reg_q;

    // c0_exe112839_reg(REG,3)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe112839_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B8_branch_enable_q == 1'b1)
        begin
            c0_exe112839_reg_q <= in_c0_exe112839;
        end
    end

    // out_c0_exe112839(GPOUT,28)
    assign out_c0_exe112839 = c0_exe112839_reg_q;

    // c0_exe1228411_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe1228411_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B8_branch_enable_q == 1'b1)
        begin
            c0_exe1228411_reg_q <= in_c0_exe1228411;
        end
    end

    // out_c0_exe1228411(GPOUT,29)
    assign out_c0_exe1228411 = c0_exe1228411_reg_q;

    // c0_exe1328513_reg(REG,5)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe1328513_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B8_branch_enable_q == 1'b1)
        begin
            c0_exe1328513_reg_q <= in_c0_exe1328513;
        end
    end

    // out_c0_exe1328513(GPOUT,30)
    assign out_c0_exe1328513 = c0_exe1328513_reg_q;

    // c0_exe1428615_reg(REG,6)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe1428615_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (stencil_2d_B8_branch_enable_q == 1'b1)
        begin
            c0_exe1428615_reg_q <= in_c0_exe1428615;
        end
    end

    // out_c0_exe1428615(GPOUT,31)
    assign out_c0_exe1428615 = c0_exe1428615_reg_q;

    // c0_exe92813_reg(REG,8)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe92813_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (stencil_2d_B8_branch_enable_q == 1'b1)
        begin
            c0_exe92813_reg_q <= in_c0_exe92813;
        end
    end

    // out_c0_exe92813(GPOUT,32)
    assign out_c0_exe92813 = c0_exe92813_reg_q;
