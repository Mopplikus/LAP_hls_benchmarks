...
`define READ_LATENCY 0  //the read latency of the agent BFM
`define INDEX_ZERO 0  //always refer to index zero for non-bursting transactions
`default_nettype none

module hls_sim_mm_agent_dpi_bfm(
                                  // clock, reset
                                  clock,
                                  reset_n,
   ...
   import "DPI-C" context function int __ihc_hls_mm_agent_read  (string component_name, int id, input bit [AV_ADDRESS_W-1:0] address, output bit [AV_DATA_W-1:0] data);
   import "DPI-C" context function void __ihc_hls_mm_agent_write(string component_name, int id, input bit [AV_ADDRESS_W-1:0] address,  input bit [AV_DATA_W-1:0] data, input bit [AV_NUMSYMBOLS-1:0] byte_enable);
   import "DPI-C" context function void __ihc_hls_dbgs(string msg);

   //chandle agent_objptr;
   ...
   altera_avalon_mm_slave_bfm
     ...
     bfm (
          .clk                      (clock                  ),
          .reset                    (~reset_n               ), // mm agent bfm uses assert high reset
    ...
    //initialize the Agent BFM
    initial
    begin
      `SLV_BFM.init();
    end

    //wait for requests from the host
    ...
    begin
          ...
          agent_pop_and_get_command(request, address, burst_size, data, byte_enable);
          if (request==REQ_WRITE)
          begin
            $sformat(message, "[%7t][msim] %m: Host %s request to address %h with burst size %d, data[0] %h and byte enable[0] %h", $time, convert_to_str(request), address, burst_size, data[0], byte_enable[0]);
            __ihc_hls_dbgs(message);
          end
          else if (request==REQ_READ)
          begin
            $sformat(message, "[%7t][msim] %m: Host %s request from address %h with burst size %d", $time, convert_to_str(request), address, burst_size);
            __ihc_hls_dbgs(message);
          end

          //agent BFM responses to the host's request
          if (request==REQ_WRITE)
          begin
            //internal_mem[address] = data;
            for (int i = 0; i < burst_size; i++) begin : GEN_RANDOM_BLOCK_NAME_R108
              __ihc_hls_mm_agent_write(COMPONENT_NAME, INTERFACE_ID, address + (i << $clog2(AV_NUMSYMBOLS)), data[i], byte_enable[i]);
            end
          end
          else if (request==REQ_READ)
          begin
            ...
            for (int i = 0; i < burst_size; i++) begin : GEN_RANDOM_BLOCK_NAME_R109
              latency = __ihc_hls_mm_agent_read(COMPONENT_NAME, INTERFACE_ID, address + (i << $clog2(AV_NUMSYMBOLS)), data[i]);
            end
            if (latency <= 1) begin
              latency = 0;
            end
            ...
            agent_set_and_push_response(burst_size, data, latency);
    ...
    //this task pops the host request from queue and extract the request information
    task agent_pop_and_get_command;
    ...
    //this task sets a response as a result of host request and push it back to the host
    task agent_set_and_push_response;
