...
module covariance_B6_branch (
    ...
    wire [0:0] VCC_q;
    reg [31:0] c0_exe52639_reg_q;
    wire [0:0] c0_exe726511_cmp_q;
    ...
    // c0_exe726511_cmp(LOGICAL,3)
    assign c0_exe726511_cmp_q = ~ (in_c0_exe726511);

    // valid_out_1_and(LOGICAL,24)
    assign valid_out_1_and_q = in_valid_in & c0_exe726511_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe726511;
    ...
    // covariance_B6_branch_enable(LOGICAL,4)
    assign covariance_B6_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // c0_exe52639_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe52639_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (covariance_B6_branch_enable_q == 1'b1)
        begin
            c0_exe52639_reg_q <= in_c0_exe52639;
        end
    end

    // out_c0_exe52639(GPOUT,17)
    assign out_c0_exe52639 = c0_exe52639_reg_q;

    // covariance_B6_branch_enable_not(LOGICAL,5)
