...
module covariance_B7_branch (
    input wire [0:0] in_c0_exe4262,
    input wire [31:0] in_c0_exe5263,
    input wire [0:0] in_c0_exe6264,
    input wire [0:0] in_c0_exe7265,
    ...
    wire [0:0] VCC_q;
    wire [0:0] c0_exe4262_cmp_q;
    reg [31:0] c0_exe5263_reg_q;
    reg [0:0] c0_exe6264_reg_q;
    reg [0:0] c0_exe7265_reg_q;
    ...
    // c0_exe4262_cmp(LOGICAL,2)
    assign c0_exe4262_cmp_q = ~ (in_c0_exe4262);

    // valid_out_1_and(LOGICAL,33)
    assign valid_out_1_and_q = in_valid_in & c0_exe4262_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe4262;
    ...
    // covariance_B7_branch_enable(LOGICAL,6)
    assign covariance_B7_branch_enable_q = not_valid_or_not_stall_0_q & not_valid_or_not_stall_1_q;

    // c0_exe5263_reg(REG,3)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe5263_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (covariance_B7_branch_enable_q == 1'b1)
        begin
            c0_exe5263_reg_q <= in_c0_exe5263;
        end
    end

    // out_c0_exe5263(GPOUT,23)
    assign out_c0_exe5263 = c0_exe5263_reg_q;

    // c0_exe6264_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe6264_reg_q <= $unsigned(1'b0);
        end
        else if (covariance_B7_branch_enable_q == 1'b1)
        begin
            c0_exe6264_reg_q <= in_c0_exe6264;
        end
    end

    // out_c0_exe6264(GPOUT,24)
    assign out_c0_exe6264 = c0_exe6264_reg_q;

    // c0_exe7265_reg(REG,5)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe7265_reg_q <= $unsigned(1'b0);
        end
        else if (covariance_B7_branch_enable_q == 1'b1)
        begin
            c0_exe7265_reg_q <= in_c0_exe7265;
        end
    end

    // out_c0_exe7265(GPOUT,25)
    assign out_c0_exe7265 = c0_exe7265_reg_q;
    ...
    // covariance_B7_branch_enable_not(LOGICAL,7)
