...
module gaussian_B2_branch (
    ...
    wire [0:0] VCC_q;
    reg [31:0] add44_push172_reg_q;
    wire [0:0] cmp24528_cmp_q;
    ...
    // cmp24528_cmp(LOGICAL,3)
    assign cmp24528_cmp_q = ~ (in_cmp24528);

    // valid_out_1_and(LOGICAL,24)
    assign valid_out_1_and_q = in_valid_in & cmp24528_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_cmp24528;
    ...
    // add44_push172_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            add44_push172_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (gaussian_B2_branch_enable_q == 1'b1)
        begin
            add44_push172_reg_q <= in_add44_push172;
        end
    end

    // out_add44_push172(GPOUT,17)
    assign out_add44_push172 = add44_push172_reg_q;
