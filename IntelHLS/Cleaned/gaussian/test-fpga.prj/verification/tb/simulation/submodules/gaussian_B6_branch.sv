...
module gaussian_B6_branch (
    ...
    wire [0:0] notcmp3546_pop205_cmp_q;
    ...
    // notcmp3546_pop205_cmp(LOGICAL,16)
    assign notcmp3546_pop205_cmp_q = ~ (in_notcmp3546_pop205);

    // valid_out_1_and(LOGICAL,24)
    assign valid_out_1_and_q = in_valid_in & notcmp3546_pop205_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_notcmp3546_pop205;
    ...
    // add44_push173_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            add44_push173_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (gaussian_B6_branch_enable_q == 1'b1)
        begin
            add44_push173_reg_q <= in_add44_push173;
        end
    end

    // out_add44_push173(GPOUT,17)
    assign out_add44_push173 = add44_push173_reg_q;
