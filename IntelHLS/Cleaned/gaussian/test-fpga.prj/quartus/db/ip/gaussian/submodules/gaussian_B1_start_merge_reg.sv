...
module gaussian_B1_start_merge_reg (
    ...
    // gaussian_B1_start_merge_reg_valid_reg_not(LOGICAL,6)
    assign gaussian_B1_start_merge_reg_valid_reg_not_q = ~ (gaussian_B1_start_merge_reg_valid_reg_q);

    // stall_in_not_or_gaussian_B1_start_merge_reg_valid_reg(LOGICAL,9)
    ...
    // gaussian_B1_start_merge_reg_valid_reg(REG,4)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            gaussian_B1_start_merge_reg_valid_reg_q <= $unsigned(1'b0);
        end
        else if (stall_in_not_or_gaussian_B1_start_merge_reg_valid_reg_q == 1'b1)
        begin
            gaussian_B1_start_merge_reg_valid_reg_q <= in_valid_in;
        end
    end

    // gaussian_B1_start_merge_reg_data_reg(REG,3)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
    ...
    // gaussian_B1_start_merge_reg_valid_reg_and_stall_in(LOGICAL,5)
