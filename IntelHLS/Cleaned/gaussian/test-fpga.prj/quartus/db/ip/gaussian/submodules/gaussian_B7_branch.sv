...
module gaussian_B7_branch (
    input wire [31:0] in_add44_push17,
    input wire [0:0] in_masked,
    input wire [0:0] in_memdep,
    input wire [0:0] in_notcmp3546_pop20,
    ...
    reg [0:0] notcmp3546_pop20_reg_q;
    reg [0:0] pop19_reg_q;
    ...
    // add44_push17_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            add44_push17_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (gaussian_B7_branch_enable_q == 1'b1)
        begin
            add44_push17_reg_q <= in_add44_push17;
        end
    end

    // out_add44_push17(GPOUT,22)
    assign out_add44_push17 = add44_push17_reg_q;
    ...
    // notcmp3546_pop20_reg(REG,21)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            notcmp3546_pop20_reg_q <= $unsigned(1'b0);
        end
        else if (gaussian_B7_branch_enable_q == 1'b1)
        begin
            notcmp3546_pop20_reg_q <= in_notcmp3546_pop20;
        end
    end

    // out_notcmp3546_pop20(GPOUT,24)
    assign out_notcmp3546_pop20 = notcmp3546_pop20_reg_q;
