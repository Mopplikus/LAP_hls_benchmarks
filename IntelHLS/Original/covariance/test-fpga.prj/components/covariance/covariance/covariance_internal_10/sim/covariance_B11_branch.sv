...
module covariance_B11_branch (
    ...
    wire [0:0] VCC_q;
    wire [0:0] c0_exe1242432_cmp_q;
    reg [0:0] c0_exe441624_reg_q;
    reg [31:0] c0_exe842028_reg_q;
    reg [0:0] c1_exe1369_reg_q;
    ...
    // c0_exe1242432_cmp(LOGICAL,2)
    assign c0_exe1242432_cmp_q = ~ (in_c0_exe1242432);

    // valid_out_1_and(LOGICAL,30)
    assign valid_out_1_and_q = in_valid_in & c0_exe1242432_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe1242432;
    ...
    // c0_exe441624_reg(REG,3)
    always @ (posedge clock)
    begin
        if (!rst_sync_rst_sclrn[0])
        begin
            c0_exe441624_reg_q <= $unsigned(1'b0);
        end
        else if (covariance_B11_branch_enable_q == 1'b1)
        begin
            c0_exe441624_reg_q <= in_c0_exe441624;
        end
    end

    // out_c0_exe441624(GPOUT,21)
    assign out_c0_exe441624 = c0_exe441624_reg_q;

    // c0_exe842028_reg(REG,4)
    always @ (posedge clock)
    begin
        if (!rst_sync_rst_sclrn[0])
        begin
            c0_exe842028_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (covariance_B11_branch_enable_q == 1'b1)
        begin
            c0_exe842028_reg_q <= in_c0_exe842028;
        end
    end

    // out_c0_exe842028(GPOUT,22)
    assign out_c0_exe842028 = c0_exe842028_reg_q;

    // c1_exe1369_reg(REG,5)
    always @ (posedge clock)
    begin
        if (!rst_sync_rst_sclrn[0])
        begin
            c1_exe1369_reg_q <= $unsigned(1'b0);
        end
        else if (covariance_B11_branch_enable_q == 1'b1)
        begin
            c1_exe1369_reg_q <= in_c1_exe1369;
        end
    end

    // out_c1_exe1369(GPOUT,23)
    assign out_c1_exe1369 = c1_exe1369_reg_q;
