...
module covariance_B6_branch (
    ...
    wire [0:0] VCC_q;
    reg [0:0] c0_exe229113_reg_q;
    reg [31:0] c0_exe429316_reg_q;
    wire [0:0] c0_exe629518_cmp_q;
    ...
    // c0_exe629518_cmp(LOGICAL,4)
    assign c0_exe629518_cmp_q = ~ (in_c0_exe629518);

    // valid_out_1_and(LOGICAL,27)
    assign valid_out_1_and_q = in_valid_in & c0_exe629518_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe629518;
    ...
    // c0_exe229113_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe229113_reg_q <= $unsigned(1'b0);
        end
        else if (covariance_B6_branch_enable_q == 1'b1)
        begin
            c0_exe229113_reg_q <= in_c0_exe229113;
        end
    end

    // out_c0_exe229113(GPOUT,19)
    assign out_c0_exe229113 = c0_exe229113_reg_q;

    // c0_exe429316_reg(REG,3)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe429316_reg_q <= $unsigned(32'b00000000000000000000000000000000);
        end
        else if (covariance_B6_branch_enable_q == 1'b1)
        begin
            c0_exe429316_reg_q <= in_c0_exe429316;
        end
    end

    // out_c0_exe429316(GPOUT,20)
    assign out_c0_exe429316 = c0_exe429316_reg_q;
