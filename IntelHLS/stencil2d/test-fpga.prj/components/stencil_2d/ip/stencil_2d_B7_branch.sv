...
module stencil_2d_B7_branch (
    ...
    wire [0:0] VCC_q;
    reg [0:0] c0_exe51926_reg_q;
    wire [0:0] c0_exe61938_cmp_q;
    ...
    // c0_exe61938_cmp(LOGICAL,3)
    assign c0_exe61938_cmp_q = ~ (in_c0_exe61938);

    // valid_out_1_and(LOGICAL,24)
    assign valid_out_1_and_q = in_valid_in & c0_exe61938_cmp_q;
    ...
    assign valid_out_0_and_q = in_valid_in & in_c0_exe61938;
    ...
    // c0_exe51926_reg(REG,2)
    always @ (posedge clock or negedge resetn)
    begin
        if (!resetn)
        begin
            c0_exe51926_reg_q <= $unsigned(1'b0);
        end
        else if (stencil_2d_B7_branch_enable_q == 1'b1)
        begin
            c0_exe51926_reg_q <= in_c0_exe51926;
        end
    end

    // out_c0_exe51926(GPOUT,15)
    assign out_c0_exe51926 = c0_exe51926_reg_q;
