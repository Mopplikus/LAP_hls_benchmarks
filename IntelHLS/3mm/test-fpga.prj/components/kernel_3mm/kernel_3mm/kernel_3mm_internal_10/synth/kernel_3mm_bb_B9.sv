// ------------------------------------------------------------------------- 
// High Level Design Compiler for Intel(R) FPGAs Version 21.4 (Release Build #53.1)
// 
// Legal Notice: Copyright 2021 Intel Corporation.  All rights reserved.
// Your use of  Intel Corporation's design tools,  logic functions and other
// software and  tools, and its AMPP partner logic functions, and any output
// files any  of the foregoing (including  device programming  or simulation
// files), and  any associated  documentation  or information  are expressly
// subject  to the terms and  conditions of the  Intel FPGA Software License
// Agreement, Intel MegaCore Function License Agreement, or other applicable
// license agreement,  including,  without limitation,  that your use is for
// the  sole  purpose of  programming  logic devices  manufactured by  Intel
// and  sold by Intel  or its authorized  distributors. Please refer  to the
// applicable agreement for further details.
// ---------------------------------------------------------------------------

// SystemVerilog created from bb_kernel_3mm_B9
// Created for function/kernel kernel_3mm
// SystemVerilog created on Fri Apr  7 18:25:08 2023


(* altera_attribute = "-name AUTO_SHIFT_REGISTER_RECOGNITION OFF; -name MESSAGE_DISABLE 10036; -name MESSAGE_DISABLE 10037; -name MESSAGE_DISABLE 14130; -name MESSAGE_DISABLE 14320; -name MESSAGE_DISABLE 15400; -name MESSAGE_DISABLE 14130; -name MESSAGE_DISABLE 10036; -name MESSAGE_DISABLE 12020; -name MESSAGE_DISABLE 12030; -name MESSAGE_DISABLE 12010; -name MESSAGE_DISABLE 12110; -name MESSAGE_DISABLE 14320; -name MESSAGE_DISABLE 13410; -name MESSAGE_DISABLE 113007; -name MESSAGE_DISABLE 10958" *)
module kernel_3mm_bb_B9 (
    input wire [0:0] in_feedback_in_21,
    output wire [0:0] out_feedback_out_17,
    input wire [0:0] in_feedback_stall_in_17,
    output wire [0:0] out_feedback_stall_out_21,
    input wire [0:0] in_feedback_valid_in_21,
    output wire [0:0] out_feedback_valid_out_17,
    input wire [0:0] in_c0_exe449926_0,
    input wire [0:0] in_memdep_134_0,
    input wire [0:0] in_stall_in_0,
    input wire [0:0] in_valid_in_0,
    output wire [0:0] out_memdep_phi7_pop21,
    output wire [0:0] out_stall_out_0,
    output wire [0:0] out_valid_out_0,
    input wire clock,
    input wire resetn
    );

    wire [0:0] bb_kernel_3mm_B9_stall_region_out_feedback_out_17;
    wire [0:0] bb_kernel_3mm_B9_stall_region_out_feedback_stall_out_21;
    wire [0:0] bb_kernel_3mm_B9_stall_region_out_feedback_valid_out_17;
    wire [0:0] bb_kernel_3mm_B9_stall_region_out_memdep_phi7_pop21;
    wire [0:0] bb_kernel_3mm_B9_stall_region_out_stall_out;
    wire [0:0] bb_kernel_3mm_B9_stall_region_out_valid_out;
    wire [0:0] kernel_3mm_B9_branch_out_memdep_phi7_pop21;
    wire [0:0] kernel_3mm_B9_branch_out_stall_out;
    wire [0:0] kernel_3mm_B9_branch_out_valid_out_0;
    wire [0:0] kernel_3mm_B9_merge_out_c0_exe449926;
    wire [0:0] kernel_3mm_B9_merge_out_memdep_134;
    wire [0:0] kernel_3mm_B9_merge_out_stall_out_0;
    wire [0:0] kernel_3mm_B9_merge_out_valid_out;


    // kernel_3mm_B9_branch(BLACKBOX,13)
    kernel_3mm_B9_branch thekernel_3mm_B9_branch (
        .in_memdep_phi7_pop21(bb_kernel_3mm_B9_stall_region_out_memdep_phi7_pop21),
        .in_stall_in_0(in_stall_in_0),
        .in_valid_in(bb_kernel_3mm_B9_stall_region_out_valid_out),
        .out_memdep_phi7_pop21(kernel_3mm_B9_branch_out_memdep_phi7_pop21),
        .out_stall_out(kernel_3mm_B9_branch_out_stall_out),
        .out_valid_out_0(kernel_3mm_B9_branch_out_valid_out_0),
        .clock(clock),
        .resetn(resetn)
    );

    // kernel_3mm_B9_merge(BLACKBOX,14)
    kernel_3mm_B9_merge thekernel_3mm_B9_merge (
        .in_c0_exe449926_0(in_c0_exe449926_0),
        .in_memdep_134_0(in_memdep_134_0),
        .in_stall_in(bb_kernel_3mm_B9_stall_region_out_stall_out),
        .in_valid_in_0(in_valid_in_0),
        .out_c0_exe449926(kernel_3mm_B9_merge_out_c0_exe449926),
        .out_memdep_134(kernel_3mm_B9_merge_out_memdep_134),
        .out_stall_out_0(kernel_3mm_B9_merge_out_stall_out_0),
        .out_valid_out(kernel_3mm_B9_merge_out_valid_out),
        .clock(clock),
        .resetn(resetn)
    );

    // bb_kernel_3mm_B9_stall_region(BLACKBOX,2)
    kernel_3mm_bb_B9_stall_region thebb_kernel_3mm_B9_stall_region (
        .in_c0_exe449926(kernel_3mm_B9_merge_out_c0_exe449926),
        .in_feedback_in_21(in_feedback_in_21),
        .in_feedback_stall_in_17(in_feedback_stall_in_17),
        .in_feedback_valid_in_21(in_feedback_valid_in_21),
        .in_memdep_134(kernel_3mm_B9_merge_out_memdep_134),
        .in_stall_in(kernel_3mm_B9_branch_out_stall_out),
        .in_valid_in(kernel_3mm_B9_merge_out_valid_out),
        .out_feedback_out_17(bb_kernel_3mm_B9_stall_region_out_feedback_out_17),
        .out_feedback_stall_out_21(bb_kernel_3mm_B9_stall_region_out_feedback_stall_out_21),
        .out_feedback_valid_out_17(bb_kernel_3mm_B9_stall_region_out_feedback_valid_out_17),
        .out_memdep_phi7_pop21(bb_kernel_3mm_B9_stall_region_out_memdep_phi7_pop21),
        .out_stall_out(bb_kernel_3mm_B9_stall_region_out_stall_out),
        .out_valid_out(bb_kernel_3mm_B9_stall_region_out_valid_out),
        .clock(clock),
        .resetn(resetn)
    );

    // feedback_out_17_sync(GPOUT,4)
    assign out_feedback_out_17 = bb_kernel_3mm_B9_stall_region_out_feedback_out_17;

    // feedback_stall_out_21_sync(GPOUT,6)
    assign out_feedback_stall_out_21 = bb_kernel_3mm_B9_stall_region_out_feedback_stall_out_21;

    // feedback_valid_out_17_sync(GPOUT,8)
    assign out_feedback_valid_out_17 = bb_kernel_3mm_B9_stall_region_out_feedback_valid_out_17;

    // out_memdep_phi7_pop21(GPOUT,15)
    assign out_memdep_phi7_pop21 = kernel_3mm_B9_branch_out_memdep_phi7_pop21;

    // out_stall_out_0(GPOUT,16)
    assign out_stall_out_0 = kernel_3mm_B9_merge_out_stall_out_0;

    // out_valid_out_0(GPOUT,17)
    assign out_valid_out_0 = kernel_3mm_B9_branch_out_valid_out_0;

endmodule
